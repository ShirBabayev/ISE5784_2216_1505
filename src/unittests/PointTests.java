...
class PointTests {
	...
	void testSubtract() {
		...
		assertEquals(p2.subtract(p1),v1, "ERROR: (point2 - point1) does not work correctly");
		 // TC12: test zero vector Exception accepted from Subtracting a point from itself:
	...
	void testDistance() {
		...
		assertEquals(Point.ZERO,p1.distance(p1),"ERROR: point distance to itself is not zero");
		 // TC22:  test that distance between a point to itself returns the right result (ZERO)
	...
	void testDistanceSquared() {
		...
		assertEquals(Point.ZERO,p1.distanceSquared(p1),"ERROR: point squared distance to itself is not zero");
		 // TC32:  test that when squared distance between points is wrong it returns the right result
