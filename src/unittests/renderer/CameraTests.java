...
class CameraTests {
	...
	public void depthOfFieldEffect() {
		...
		scene.lights.add(new SpotLight(new Color(700, 400, 400), new Point(0, 1500, 0), new Vector(500, -100, 0))
		 ...
		 cameraBuilder.setLocation(new Point(-500, 0, 0)) 
		 .setVpDistance(1000).setVpSize(400, 400).setDirection(new Vector(1, 0, 0), new Vector(0, 1, 0)) 
 			.setRayTracer(new SimpleRayTracer(scene))
		 .setImageWriter(new ImageWriter("noDepthOfFieldEffect", 600, 600)).build().renderImage().writeToImage(); 

	    cameraBuilder.setFocalDistance(1200).setSqrtGridSize(2).setApertureRadius(2)
...
/*	
	public void test() {
		Scene s=new Scene("new");
		s.geometries.add(new Sphere(new Point(20,0,0), 1).setEmission(new Color(RED)).setMaterial(new Material().setKd(0.2).setKs(0.2).setShininess(30)), 
				new Sphere(new Point(30,0,0), 2).setEmission(new Color(RED)).setMaterial(new Material().setKd(0.2).setKs(0.2).setShininess(30)), 
				new Sphere(new Point(40,0,0), 3).setEmission(new Color(RED)).setMaterial(new Material().setKd(0.2).setKs(0.2).setShininess(30)));
		s.setAmbientLight(new AmbientLight(new Color(255, 255, 255), 0.1));
		s.lights.add(new SpotLight(new Color(700, 400, 400), new Point(0,100,0),new Vector(0,-70,0)));
		Camera.Builder cameraBuilder = Camera.getBuilder()
				.setRayTracer(new SimpleRayTracer(new Scene("new"))).setImageWriter(new ImageWriter("new", 1, 1))
				.setLocation(Point.ZERO).setDirection(new Vector(1, 0, 0), new Vector(0, 1, 0)).setVpDistance(10).setApertureRadius(3).setSqrtGridSize(2).setFocalDistance(35).setVpSize(40, 40);
	...
	public void advancedDepthOfFieldTest() {
       ...
       Camera.Builder cameraBuilder = Camera.getBuilder()
        ...
        cameraBuilder.setImageWriter(new ImageWriter("advanced_depth", 600, 600))
                ...
                .setApertureRadius(0.3)  // Decreased aperture to reduce overall blurriness while still showing depth of field
