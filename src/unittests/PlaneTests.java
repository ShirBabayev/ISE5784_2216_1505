...
class PlaneTests {
	...
	void testGetNormal() {
		...
		/*TC03: Checks that the normal is indeed perpendicular to the vectors on the plane*/
		Vector result = plane1.getNormal();
		for (int i = 0; i < 3; ++i)
			/*Each time creates a vector from 2 points on the plane and checks that the normal is indeed perpendicular to this vector*/
	...
	void testGetNormalPoint() {
		...
		/*TC05: Checks that the function does indeed return a correct result*/
		assertEquals(new Vector(d,d,d), plane1.getNormal(pts[0]), "ERROR: normal of the plane does not work correctly");
		/*TC06: Checks that the normal is indeed perpendicular to the vectors on the plane*/
