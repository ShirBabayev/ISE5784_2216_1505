...
class GeometriesTests {
	...
	void testFindIntersections() {
		...
		assertNull(g1.findIntersections(new Ray(new Point(0, 0, 0), new Vector(1, 1, 1))),
				"ERROR: The result for a collection with no Geometries is not null");
		...
		assertNull(g1.findIntersections(new Ray(new Point(0, 0, 0), new Vector(1, 0, 0))),
				"ERROR: The intersection returns wrong value");

		// TC03: There are intersection points with only one body
		g2.add(new Sphere(new Point(1, 0, 0), 1), new Sphere(new Point(10, 0, 0), 1));
		assertEquals(2, g2.findIntersections(new Ray(new Point(1, 0, -2), new Vector(0, 0, 1))).size(),
				"ERROR: he intersection with only one body returns wrong value");

		// TC04: There are intersections with all the geometries
		assertEquals(4, (g2.findIntersections(new Ray(new Point(-1, 0, 0), new Vector(1, 0, 0))).size()),
				"ERROR: the intersection with all bodies returns wrong value");
		...
		assertEquals(4, (g2.findIntersections(new Ray(new Point(-1, 0, 0), new Vector(1, 0, 0))).size()),
				"ERROR: he intersection with 2 bodies returns wrong value");
