...
class GeometriesTests {
	...
	void testFindIntersections() {
		...
		assertNull(g1.findIntersections(new Ray(new Vector(1, 1, 1), new Point(0, 0, 0))),
		...
		assertNull(g1.findIntersections(new Ray(new Vector(1, 0, 0), new Point(0, 0, 0))),
		...
		assertEquals(2, g2.findIntersections(new Ray(new Vector(0, 0, 1), new Point(1, 0, -2))).size(),
				"ERROR: he intersection with only one body returns wrong value");

		// TC04: There are intersections with all the geometries
		assertEquals(4, (g2.findIntersections(new Ray(new Vector(1, 0, 0), new Point(-1, 0, 0))).size()),
