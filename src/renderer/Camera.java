...
public class Camera implements Cloneable {
	 ...
	 * This is used to determine the focus point for depth of field calculations.
	 */
	private double focalDistance = 0;

	/**
	 * The radius of the camera's aperture.
	 * A non-zero aperture radius will result in depth of field effects.
	 */
	private double apertureRadius = 0;

	/**
	 * The square root of the grid size used for anti-aliasing.
	 * This determines the number of rays cast per pixel for better image quality.
	 */
	private int sqrtGridSize = 1;

	/**
	 * The list of points on the aperture.
	 * These points are used to construct rays for depth of field calculations.
	 */
	 ...
	 * @param source   The source point of the ray, typically the camera position.
	 * @param distance The distance from the source to the view plane.
	...
	/**
	 * Casts rays through a specific pixel on the image.
	 * Depending on the aperture radius, it can handle depth of field.
	 *
	...
	public static class Builder {
		...
		/**
		 * Sets the square root of the grid size for the camera.
		 * The amount of squares that will be in the aperture window in length and width.		 *
		...
		/**
		 * A function that creates a given amount of points randomly distributed around the center point 
		 * of the camera in a given radius range
		 * @param radius is radius of the aperture window around the center of the camera
		 * @param gridSize is the amount of squares we will create in the length and width of the aperture window. There will be gridSize*gridSize squares in total
		 * @return A list of points within the aperture window that fall within the given radius around the center of the camera
		 */
		private List<Point> constructBeamPoints(double radius, int gridSize) {
		    ...
		    for (int i = 0; i < gridSize; i++) {
		        for (int j = 0; j < gridSize; j++) {
		            ...
		            p = camera.p0;
		            p = p.add(camera.vRight.scale(-radius + i * gridStep + random.nextFloat() * gridStep));
		            p = p.add(camera.vUp.scale(-radius + j * gridStep + random.nextFloat() * gridStep));
		    ...
		    // Filter the points to keep only those inside the circle of radius `radius` around P0
